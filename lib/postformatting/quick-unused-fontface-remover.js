...
function unusedFontfaceRemover (css) {
  ...
  while ((ff = fontFaceRegex.exec(css)) !== null) {
    ...
    var fontFound = false,
      ...
      if(fontFaceAppearances[fontName] === undefined) {
        while ((m = fontNameRegex.exec(css)) !== null) {
          if (m[1].indexOf('@font-face') === -1) {
            // log('FOUND, keep rule')
            fontFound = true
            fontFaceAppearances[fontName] = true
            break
          }
        }
        if(!fontFound) {
          fontFaceAppearances[fontName] = false
        }
      } else {
        fontFound = fontFaceAppearances[fontName]
      }
