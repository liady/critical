...
function penthouseScriptArgs (options, ast, debuglog) {
  ...
  return [
    options.url || '',
    writeAstToFile(ast, debuglog, options.compress),
...
function getTempFileName(compress) {
  var postfix = compress ? '.zip' : '.txt';
  var tmpobj = tmp.fileSync({dir: TMP_DIR, prefix: 'critical_', postfix: postfix, discardDescriptor : true});
  return tmpobj.name;
}

function writeAstToFile (ast, debuglog, compress) {
  // save ast to file
  var tmpName = getTempFileName(compress);
  ...
  try {
    ...
    tmpName = getTempFileName(compress);
    fs.writeFileSync(tmpName, zippedContents)
  }
...
function writeCssToFile(css, debuglog) {
  // save css to file
  var tmpName = getTempFileName();
...
var m = module.exports = function (options, rawCallback) { // jshint ignore: line
  ...
  function generateAstFromCssContents(csscontents) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      var cssfilename = writeCssToFile(css, debuglog);
