...
function getTempFileName() {
  var tmpobj = tmp.fileSync({dir: TMP_DIR, prefix: 'critical_', discardDescriptor : true});
  return tmpobj.name;
}

function writeAstToFile (ast, debuglog) {
  // save ast to file
  var tmpName = getTempFileName();
  ast = JSON.stringify(ast);
  var zippedContents = require('./phantomjs/utils/lz').compressToUTF16(ast);
  try {
